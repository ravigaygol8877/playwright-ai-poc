...
| **Contains** | `ae-home.json`, `ae-login.json`, `ae-home-scenarios.json`, `ae-login-scenarios.json`, `test-catalog.json` |
...
| **Contains** | `interceptHelper.ts` (common login/logout flows), `commonPattern.ts` (`CorePattern` class with nav helpers), `constants.ts` (`DESKTOP_VIEW_PORT`, `MOBILE_VIEW_PORT`), `waitUtils.ts` (custom wait helpers) |
...
| `generate.ts` | `npm run generate` | Single-suite generation with verbose output |
