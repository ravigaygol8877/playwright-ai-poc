...
```
Requirements (Excel / config/platform.json)
         │
         ▼
...
Knowledge Base (pipeline/kb/pages/<page>.json)
  → Auto-generated by KnowledgeBaseGenerator (npm run kb:generate)
  → Curated and extended by QA engineers after generation
  → Injected as plain text into LLM prompts

LLM Layer
  → ProviderFactory (Gemini / GitHub Models / OpenRouter / LM Studio)
...
**How it fits:** The `ArtifactManifest` utility already exists in `pipeline/utils/ArtifactManifest.ts` — it just needs to be wired into the pipeline as a skip-guard.
...
**Why not now:** We have 10 modules. The team can hold this in their heads. Registry adds bureaucracy without benefit at current scale.
